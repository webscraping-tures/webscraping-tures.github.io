...
literal `.` in a string though, we have to escape the dot by using the
*escape character* `\` or actually two of those, telling R to take the next
character literally. So to match a dot, we write `"\\."`.
...
see, how reliably detecting a simple string as "raub" gets complicated very
quickly.
...
as in "*Raub*" or "*Räub*er", or where "aub" and "äub" are are prefixed with
...
expression are allowed or required too repeat. If we know the exact number of
...
*any* character -- we have to escape it by using two backslashs.
...
could be found in a string. We use the function inside of `mutate()` to assign

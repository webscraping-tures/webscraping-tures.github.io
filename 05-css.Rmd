...
CSS Selectors kommen in rvest als Argument der Funktion `read_html()`
zum Einsatz. Als zweites Argument -- das erste legt fest, auf welche
Daten die Funktion angewandt werden soll -- geben wir einen Selector
in der Form `css = "selector"` an. Dieser bestimmt welche Elemente des
HTML-Codes wir extrahieren möchten.  Wichtig ist dabei, dass immer der
gesamte Selector -- unabhängig davon, aus wie vielen einzelnen Teilen
dieser besteht -- als String an das Argument übergeben wird.
...
Grundsätzlich ist das Konstruieren von CSS Selectors immer ein Balanceakt
zwischen Funktionalität und Lesbarkeit. Der Selector sollte genau nur die
Elemente auswählen, an denen wir interessiert sind, und gleichzeitig
verständlich sein. Letzteres ist vor allem wichtig, wenn Andere -- oder man
selbst einige Wochen später -- den Code lesen. Dabei bedeutet Lesbarkeit auch,
eine Balance zwischen Länge und Klarheit des Selectors zu erreichen.
...
dynamischen Gestaltung von Websites oder in HTML-Formularen eingesetzt. Wir
können auch sie nutzen können um einzelne Elemente gezielt zu extrahieren.
...
relvant sind, existieren die oben vorgestellten *shortcuts*, um diese schnell 
...
Im unteren Bereich unseres Beispiel HTML Codes, finden sich drei Links `<a>`
...
Anführungszeichen eingefasst ist. Die Schreibweise
`"element[attribute="value"]"` würde in die beiden Strings
`"element[attribute="` und `"]"` und das Objekt `value` geteilt werden, was in
dieser Kombination von R nicht interpretiert werden kann. Stattdessen, verwenden
wir einfache Anführungszeichen, um den Wert des Attributs zu definieren. Dies
ist eine Konvention, die in vielen Programmierkontexten Anwendung
findet. Innerhalb doppelter Anführungszeichen nutzen wir einfache, innerhalb
einfacher doppelte, um mehrere Ebenen von Anführunszeichen realisieren zu
können.
...
Attributs einen bestimmten Anfang -- `"element[attribute^='anfang']"` -- oder 
...
`"div > span"` bereits ausreichend. Wir könnten aber expliziter werden, um
weniger fehleranfälligen Code zu schreiben. Beide `<div>` Tags ähneln sich
darin, dass ihre Klassen mit `"bundesland"` beginnen und wir wissen ja bereits,
wie wir dies ausnutzen können:
...
Möchten wir nur das letzte Kind selektieren, könnten wir dies mit `:last-child`
tun:
...
Selektoren kann auf komplexen Websites sehr schwer sein, vor allem wenn der
HTML-Code nicht so übersichtlich formatiert ist, wie in unserem Beispiel.  Hier
kann es hilfreich sein, die in modernen Browsern integrierten
...
Je nach Einstellung öffnen sich die Developer Tools in einem horizontal oder 
...
Für das oben ausgewählte Element, also den Ländernamen für Baden-Württemberg, 
...
Namenselementen, kann uns dies Ansatzpunkte geben, um unseren eigenen Selector
zu formulieren. Eine weitere Möglichkeit wäre es, statt der integrierten
Developer Tools, die Erweiterung *SelectorGadget* zu nutzen. Diese ermöglicht
unter anderem die Auswahl mehrere Elemente gleichzeitig. Informationen zur

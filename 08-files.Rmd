...
need to scrape the data again. Most data files you will encounter online are CSV
files. We will first talk briefly about what CSV files are and how we can use
them in R.
...
The first row usually represents the column names, as in the example above
...
columns, which we can change later on.
...
gender, year of birth, and additional indicators. 
...
First, we should download the HTML file containing the links to the CSV files
...
as one of its input arguments, the function will repeat the string connection
...
We have to specify an URL as its first argument as well as a path and file name
...
Secondly, we see that the first 8 lines are neither column names nor data, but
...
change all the time. The data you scraped today may be changed or even gone
tomorrow. Also if the structure of the site changes, your CSS selectors and URLs
may not work anymore. 

So for both reasons, it may be appropriate to save all of our downloaded HTML
data locally so that we can reload it for re-runs of our script without having
to worry about changes to the source.
...
by R. It takes the object to be saved as its first argument and the path and 
...
Please note, that `save()` does not work when saving an object to which the
...
should know that you will loose some of your work, if you try to do something 
like this:
